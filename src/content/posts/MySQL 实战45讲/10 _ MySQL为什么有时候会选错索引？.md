---
title: 10 _ MySQL为什么有时候会选错索引？
pubDate: 2024-01-01
categories: ['MySQL']
description: 10 _ MySQL为什么有时候会选错索引？
---
...
<audio><source src="https://static001.geekbang.org/resource/audio/80/5f/80f719c1c9411cf2816cd846e1e0065f.mp3" type="audio/mpeg"></audio>
...
<center><span class="reference">图1 使用explain命令查看语句执行情况</span></center>
...
<center><span class="reference">图2 session A和session B的执行流程</span></center>
...
<center><span class="reference">图3 slow log结果</span></center>
...
<center><span class="reference">图4 表t的show index 结果</span></center>
...
<center><span class="reference">图5 意外的explain结果</span></center>
...
<center><span class="reference">图6 执行analyze table t 命令恢复的explain结果</span></center>
...
<center><span class="reference">图7 a、b索引的结构图</span></center>
...
<center><span class="reference">图8 使用explain方法查看执行计划 2</span></center>
...
<center><span class="reference">图9 使用不同索引的语句执行耗时</span></center>
...
<center><span class="reference">图10 order by b,a limit 1 执行结果</span></center>
...
<center><span class="reference">图11 改写SQL的explain</span></center>

在这个例子里，我们用limit 100让优化器意识到，使用b索引代价是很高的。其实是我们根据数据特征诱导了一下优化器，也不具备通用性。

> 如果是limit 1， 因为b不用排序，从5w里只要找到一条就可以返回了，如果选择a，因为要排序，就要扫完1000条，然后才能排序，这成本明显太大，所以选择了b。 
>
> 但如果是limit 100，选择b，虽然不用排序，但找到第一条记录后，还要向右查询，看后面有没有满足条件的100个记录，从5w中找100个的成本就大于从1000找100个再加排序的成本了，所以选择a。其实limit 20就会选择a了
...
> @某、人 把02篇的redo log更新细节和change buffer的更新串了起来；<br>
> 
